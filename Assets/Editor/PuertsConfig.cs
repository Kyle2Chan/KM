...
public class PuertsConfig
{
	...
	static string[] GenerateDirectory()
	{
		return new[]
		{
			PathDefine.PATH_CS_DECLARE,
			PathDefine.PATH_CS_WRAP
		};
	}
	...
	static bool Filter(MemberInfo memberInfo)
	{
		...
		{
			{
				"UnityEngine.MonoBehaviour", new HashSet<string>()
				{
					"runInEditMode"
				}
			},
			{
				"UnityEngine.Input", new HashSet<string>()
				{
					"IsJoystickPreconfigured"
				}
			},
			{
				"UnityEngine.Texture", new HashSet<string>()
				{
					"imageContentsHash"
				}
			},
			{
				"tiny.Utils", new HashSet<string>()
				{
