    ...
    namespace Generator {
        ...
        class Menu {
            ...
            public static void GenerateCode()
            {
                ...
                string declareSave; 
                string wrapSave;
                Configure.GetCodeOutputDirectory(out declareSave, out wrapSave);
                Directory.CreateDirectory(declareSave);
                Directory.CreateDirectory(wrapSave);
                GenerateCode(declareSave, wrapSave, false, true);
            ...
            public static void GenerateDTS()
            {
                ...
                Configure.GetCodeOutputDirectory(out declareSave, out wrapSave);
                Directory.CreateDirectory(declareSave);
                GenerateCode(declareSave, wrapSave, true);
                Debug.Log("finished! use " + (DateTime.Now - start).TotalMilliseconds + " ms");
                AssetDatabase.Refresh();
            }
            ...
            public static void GenerateDTSESM()
            {
                ...
                Configure.GetCodeOutputDirectory(out declareSave, out wrapSave);
                Directory.CreateDirectory(declareSave);
                GenerateCode(declareSave, wrapSave, true, true);
                Debug.Log("finished! use " + (DateTime.Now - start).TotalMilliseconds + " ms");
                AssetDatabase.Refresh();
            }
            ...
            public static void ClearAll()
            {
                ...
                string wrapSave;
                Configure.GetCodeOutputDirectory(out declareSave, out wrapSave);
                if (Directory.Exists(declareSave))
                {
                    Directory.Delete(declareSave, true);
                    AssetDatabase.DeleteAsset(declareSave.Substring(declareSave.IndexOf("Assets") + "Assets".Length));
                }
                if (Directory.Exists(wrapSave))
                {
                    Directory.Delete(wrapSave, true);
                    AssetDatabase.DeleteAsset(wrapSave.Substring(wrapSave.IndexOf("Assets") + "Assets".Length));
                }
                AssetDatabase.Refresh();
            }

            public static void GenerateCode(string declareSave, string wrapSave, bool tsOnly = false, bool esmMode = false)
            {
                ...
                using (var jsEnv = new JsEnv())
                {
                    ...
                    if (!tsOnly)
                    {
                        ...
                        foreach (var type in genTypes)
                        {
                            ...
                            string filePath = wrapSave + typeGenInfo.WrapClassName + ".cs";

                            int uniqueId = 1;
                            while (makeFileUniqueMap.ContainsKey(filePath.ToLower()))
                            {
                                filePath = wrapSave + typeGenInfo.WrapClassName + "_" + uniqueId + ".cs";
                                uniqueId++;
                            }
                        ...
                        using (StreamWriter textWriter = new StreamWriter(wrapSave + "AutoStaticCodeRegister.cs", false, Encoding.UTF8))
                        {
                    ...
                    using (StreamWriter textWriter = new StreamWriter(declareSave + "index.d.ts", false, Encoding.UTF8))
                    {
